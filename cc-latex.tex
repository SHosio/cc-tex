...
%   2. Add \input{cc-latex} in your preamble (after \usepackage{xcolor,ifthen})
%   3. Make sure xcolor, ifthen, and changes packages are available
%
% TOGGLES (set to true/false in this file to control visibility):
%   \COMMENTStrue / \COMMENTSfalse  — author comments (\simo, \bob, etc.)
%   \setboolean{coloring}{true/false} — revision coloring (\rone, \rtwo)
%   \setboolean{todos}{true/false}    — todo markers (\todo)
%
% QUICK REFERENCE:
...
% TODO MARKERS
% --------------------------------------------------------------------------
% Toggle: set todos to false to hide all \todo{} text

\newboolean{todos}
\setboolean{todos}{true} % Change to false to hide TODOs
...
% REVISION COLORING
% --------------------------------------------------------------------------
% Toggle: set coloring to false to hide revision colors
% Use \rone{} for round 1 revisions, \rtwo{} for round 2

\newboolean{coloring}
\setboolean{coloring}{true} % Change to false to disable coloring

\newcommand{\rone}[1]{%
  \ifthenelse{\boolean{coloring}}{%
    {\color{blue}#1}%
  }{#1}%
}
